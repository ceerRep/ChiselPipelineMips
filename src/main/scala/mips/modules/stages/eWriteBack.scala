...
class eWriteBack extends Module {
  val io = IO(new Bundle {
    val pipelineMemoryResult = Input(new PipelineMemoryResult)
  ...
  val stall = io.pipelineMemoryResult.bubbled

  io.wRegId := io.pipelineMemoryResult.wRegId
  io.wRegEnabled := io.pipelineMemoryResult.controlSignal.wRegEnabled && !stall
